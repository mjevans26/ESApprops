output$appropriation <- renderPlotly({
  plot_ly(funding, x = ~Year, y = ~count)%>%
    ...
    add_trace(data = funding, x = ~Year, y = ~ConsultOld*CF2016/Species, type = "scatter", mode = "lines",
              name = "Consultation<br>(1985 - 2015)", line = list(color = "purple"), text = ~paste("$",ConsultOld,"budgeted for<br>Consultation in", Year, sep=" "), hoverinfo = "text")%>%

    add_trace(data = funding, x = ~Year, y = ~ConsultNew*CF2016/Species, type = "scatter", mode = "lines",
              name = "Consultation 2017", line = list(color = "orange"), text = ~paste("$",ConsultNew,"budgeted for<br>Consultation in", Year, sep=" "), hoverinfo = "text", visible = F)%>%

    add_trace(data = funding, x = ~Year, y = ~ESP*CF2016/Species, type = "scatter", mode = "lines",
              name = "Endangered Species<br>(1985 - 2015)", line = list(color = "blue"), text = ~paste("$",ESP,"budgeted for<br>Endangered Species Programs in", Year, sep=" "), hoverinfo = "text")%>%
