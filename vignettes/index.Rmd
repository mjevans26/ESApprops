...
Congress has provided only a small fraction of the funds needed for recovery. Data from Gerber (2016) shown here illustrate this shortchanging. From 1980 to 2014, the vast majority of listed species with recovery plans were underfunded - receiving less than 90% of the amount needed for recovery. Because funding is correlated with improvement in species status, only 15% of species improved in status from 1989 - 2011.
  ...
  fluidRow(
    column(6,
      ...
      add_pie(
        ...
        labels = c("Under (<0.90)", "Adequate", "Over (>2x)"),
        values = ~ n,
        name = "Funding",
    ...
    column(6,
      ...
      add_trace(
        ...
        x = ~change, 
        y = ~ count, 
        color = ~paste(O_U, "funded"), 
        colors = ~color,
        type = "bar",
        marker = list(line = list(color = "black", width = 2)),
        text = ~paste(count, O_U, "funded species", change),
        hoverinfo = "text") %>%
      layout(
        hovermode = "closest", 
        font = list(color = "black"), 
        barmode = "stack",
          ...
          xaxis = list(title = "Recovery Status (1989 - 2011)", 
...
Not only has Congress underfunded recovery for decades, but the average dollar per species has decreased since 2010. This drop has occurred across all endangered species programs at FWS. This decline is the result of the ESA budget consistently decreasing, combined with the continued increase in the number of listed species. 
